...
def palettize_sprite(sprite_image: Image.Image, palette: list[tuple[int, int, int]]) -> Image.Image:
    rgba = sprite_image.convert("RGBA")
    out = Image.new("P", rgba.size, 0)
    out.putpalette(palette_bytes(palette))
    pixels = []
    for pixel in rgba.getdata():
        pixels.append(0 if pixel[3] == 0 else nearest_palette_index(pixel, palette))
    out.putdata(pixels)
    return out
...
def background_card(palette: list[tuple[int, int, int]]) -> Image.Image:
    out = Image.new("P", (WALL_CARD_WIDTH, WALL_CARD_HEIGHT), 0)
    out.putpalette(palette_bytes(palette))
    split = int(WALL_CARD_HEIGHT * 0.43)
    pixels = []
    for y in range(WALL_CARD_HEIGHT):
        if y < split:
            # Blue sky ramp without a high-contrast horizon stripe.
            t = y / max(1, split - 1)
            value = 14 if t < 0.55 else 6
        else:
            # Floor ramp: dark at the horizon, warmer/brighter near the player.
            t = (y - split) / max(1, WALL_CARD_HEIGHT - split - 1)
            value = 9 if t < 0.25 else (10 if t < 0.58 else (11 if t < 0.86 else 12))
...
def sprite_lump_lookup(wad: Wad) -> dict[str, bytes]:
    return {lump.name: wad.read_lump(lump) for lump in wad.namespace_lumps("sprites")}


def sprite_cards_from_lump(
    name: str,
    data: bytes,
    doom_palette: list[tuple[int, int, int]],
    card_palette: list[tuple[int, int, int]],
    target_height: int,
) -> tuple[list[Image.Image], dict]:
    rgba, info = render_picture(data, doom_palette)
    if target_height <= 0:
        target_height = rgba.height
    resampling = getattr(getattr(Image, "Resampling", Image), "NEAREST", Image.NEAREST)
    if rgba.height != target_height:
        target_width = max(1, int(math.ceil(rgba.width * (target_height / rgba.height))))
        rgba = rgba.resize((target_width, target_height), resample=resampling)
    paletted = palettize_sprite(rgba, card_palette)
    card_count = max(1, math.ceil(paletted.width / WALL_CARD_WIDTH))
    cards = []
    for index in range(card_count):
        card = Image.new("P", (WALL_CARD_WIDTH, WALL_CARD_HEIGHT), 0)
        card.putpalette(palette_bytes(card_palette))
        left = index * WALL_CARD_WIDTH
        crop = paletted.crop((left, 0, min(left + WALL_CARD_WIDTH, paletted.width), paletted.height))
        card.paste(crop, (0, 0))
        for x in range(WALL_CARD_WIDTH):
            card.putpixel((x, WALL_CARD_HEIGHT - 1), 0)
        cards.append(card)
    return cards, {
        "lump": name,
        "source_width": info.width,
        "source_height": info.height,
        "target_width": paletted.width,
        "target_height": paletted.height,
...
def wall_card_header(report: dict) -> str:
    ...
    m3_groups = report.get("m3_sprite_groups", [])
    ...
    for group in m3_groups:
        ...
        lines.extend(
            [
                f"#define M3_CARD_{symbol}_BASE {group['base_card']}u",
                f"#define M3_CARD_{symbol}_COUNT {group['card_count']}u",
                f"#define M3_CARD_{symbol}_HEIGHT {group['target_height']}u",
            ]
        )
    lines.extend(["", "#endif", ""])
    return "\n".join(lines)
...
def build_wall_atlas(wad: Wad, map_name: str, palette_index: int = 0) -> tuple[Image.Image, dict]:
    ...
    sprite_lumps = sprite_lump_lookup(wad)
    ...
    m3_prepared = []
    for symbol, lump_name, target_height in M3_SPRITE_GROUPS:
        try:
            cards, meta = sprite_cards_from_lump(lump_name, sprite_lumps[lump_name], doom_palette, card_palette, target_height)
        except KeyError as exc:
            raise ValueError(f"required M3 sprite lump {lump_name} not found") from exc
        m3_prepared.append((symbol, cards, meta))
    ...
    for symbol, cards, meta in m3_prepared:
        base_card = card_cursor
        for index, card in enumerate(cards):
            atlas.paste(card, ((base_card + index) * WALL_CARD_WIDTH, 0))
        m3_records.append(
            {
                "symbol": symbol,
                "base_card": base_card,
                ...
                **meta,
                "tiles": len(cards) * WALL_CARD_TILES,
                "bytes_4bpp": len(cards) * WALL_CARD_BYTES_4BPP,
            }
        )
        card_cursor += len(cards)

    report = {
        "map": doom_map.name,
        "textures": records,
        "texture_count": len(records),
        "m3_sprite_groups": m3_records,
